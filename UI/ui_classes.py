...
class Button:
    ...
    def __init__(self, screen: pygame.surface, height: Coord, width: Coord, position: Position, text: str,
                 colour: Colour, hover_colour: Colour, text_colour: Colour, font_style: Optional[str] = None, border_colour: Optional[Colour] = None,
                 border_radius: int = 0, image: Optional[pygame.Surface] = None) -> None:
        self._screen = screen
        self._height = height
        self._width = width
    ...
    def draw(self) -> None:
        ...
        if self._image is not None:
            img_x = self.position[0] + (self._width - self._image.get_width()) / 2
            img_y = self.position[1] + (self._height - self._image.get_height()) / 2
            self._screen.blit(self._image, (img_x, img_y))
...
class FivePointGraph:
    ...
    def draw(self):
        ...
        for i, title in enumerate(legend_title):
            ...
            self.screen.blit(text, legend_points[i])
...
class AnimeSpotlight:
    ...
    def __init__(self, screen: pygame.Surface, background_colour: Colour,
        ...
        self.position = (0, screen.get_height() * top_bar_height_percentage)
        ...
        self.graph = FivePointGraph(screen=screen,
                                    ...
                                    position=(self.margin, self.position[1] + self.title_height),
    ...
    def format_and_display_title(self):  # TODO fix the fact that it starts out big and how it doesnt default back to 32
    ...
    def adjust_font_size(self, lines: list[str]) -> None:
        ...
        print(f"title height: {self.title_height}")
        i = 0
    ...
    def display_formatted_text(self, lines: list[str]) -> None:
        ...
        for index, text_render in text_renders.items():
            title_surf.blit(text_render, ((title_surf.get_width() - text_render.get_width()) / 2,
                                          ...
                                          (title_surf.get_height() - text_render.get_height()) / 2))
...
class RecommendationDisplay:
    ...
    def __init__(self, screen: pygame.Surface, top_bar_height_percentage: float,
        ...
        self.screen = screen
        self.position = (screen.get_width() * (1 - width_percentage), screen.get_height() * top_bar_height_percentage)
        self.height = screen.get_height() * (1 - top_bar_height_percentage)
        self.width = screen.get_width() * width_percentage
...
class PreferenceMeterDisplay:
    ...
    def draw_meter_titles(self) -> None:
        ...
        for i, meter_name in enumerate(meter_names):
            position_x = self.position[0] + i * self.width * 5 / 2
            text = self.font.render(meter_name, True, (40, 40, 240))
            offset_x = text.get_width() / 2
            self.screen.blit(text, (position_x + self.width / 2 - offset_x, self.text_height))
...
class GenreFilterDisplay:
    ...
    def __init__(self, screen: pygame.surface, pos_x_percentage: float, height_percentage: float,
        ...
        base_y = self.position[1] + self.height * 1.5 - 1
...
class TextInputBox(Button):
    ...
    def __init__(self, screen: pygame.Surface, height: Coord, width: Coord, position: Position,
        ...
        Button.__init__(self, screen, height, width, position, '', (0, 0, 0), (0, 0, 0), (0, 0, 0), border_colour=border_colour)
        self.input_text = ''
        self.font = font
