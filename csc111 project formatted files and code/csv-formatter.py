...
PROJECT_FILE_PATH = "/group assignment/csc111 project formatted files and code"
...
def read_and_write_reviews():
    ...
    with open(f"{PROJECT_FILE_PATH}/data/original_data/reviews(accidentally edited).csv", 'r', encoding="utf-8") as reader:
        line = reader.readline()
        line = line[:-9]
        counter = 0
        while line != '':
...
def remove_anime_duplicates() -> list[tuple[str]]:
    ...
    """
    with open(f"{PROJECT_FILE_PATH}/data/formatted/animes_formatted.csv", 'r', errors="ignore", encoding='utf-8') as read_obj:
